...
class RAGChatbot:
    ...
    def query(self, question: str, 
        ...
        # ---------------- 🔥 SMART LOGIC ----------------
        is_generic = self._is_generic_question(original_question)
        is_strong_context = self._is_context_strong(filtered_sims)
        
        has_relevant_context = (
            len(filtered_docs) > 0
            and is_strong_context
            and not is_generic
        )
        
        # ---------------- 🔥 GENERIC HANDLING ----------------
        if is_generic:
            _safe_print("⚠️ Generic question detected — asking clarification")
            
            clarification = (
                "Could you please specify what you are referring to? "
                "For example, mention the document name, section, or topic."
            )
            
            self._update_conversation_memory(thread_id, original_question, clarification)
            return clarification, []
        
        # ---------------- RESPONSE BUILD ----------------
        if has_relevant_context:
            _safe_print(f"📄 Using {len(filtered_docs)} strong context chunks")
            ...
            _safe_print("⚠️ Weak or no context")
    ...
    def _is_generic_question(self, question: str) -> bool:
        ...
        # Very strict generic patterns only
        generic_patterns = [
            "what is this",
            "explain this",
            "tell me about this",
        ...
        # Only block if BOTH:
        # 1. Very short
        # 2. No meaningful keywords
        words = q.split()

        if len(words) <= 3:
            return True

        # If it contains actual domain keywords → NOT generic
        important_keywords = [
            "engine", "system", "report", "result",
            "finding", "experiment", "analysis",
            "model", "data"
        ]

        if any(k in q for k in important_keywords):
            return False
    ...
    def _is_context_strong(self, similarities: list) -> bool:
        if not similarities:
            return False

        avg_sim = sum(similarities) / len(similarities)
        return avg_sim >= 0.65   # tune later
